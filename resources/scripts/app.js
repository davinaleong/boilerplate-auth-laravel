...
function main() {
    ...
    const tableFormEls = document.querySelectorAll(
        `[${dataElementAttr}="table-form"]`
    );
    ...
    changeBackground(landingEl, landingBgs);

    // Random Login Page BG
    changeBackground(loginHeadingEl, landingBgs);

    // Random Auth Page BG
    changeBackground(authEl, authBgs);
    ...
    // Added events to buttons in Form Table
    if (tableFormEls && tableFormEls.length > 0) {
        tableFormEls.forEach(function (tableFormEl) {
            const userNameEl = tableFormEl.querySelector(
                `[${dataElementAttr}="user-name"]`
            );
            const thUserEl = tableFormEl.querySelector(
                `[${dataElementAttr}="th-user"`
            );
            const thNameEl = tableFormEl.querySelector(
                `[${dataElementAttr}="th-name"`
            );
            const thKeyEl = tableFormEl.querySelector(
                `[${dataElementAttr}="th-key"`
            );
            const thValueEl = tableFormEl.querySelector(
                `[${dataElementAttr}="th-value"`
            );
            const tbodyEl = tableFormEl.querySelector(`tbody`);
            const btnAddEl = tableFormEl.querySelector(
                `[${dataElementAttr}="btn-add"]`
            );
            ...
            if (tbodyEl && btnAddEl) {
                btnAddEl.addEventListener(`click`, function (event) {
                    ...
                    tbodyEl.innerHTML += `
                    <tr>
                      <td data-cell="${thUserEl.innerText}">
                        ${userNameEl.innerText}
                      </td>
                      <td data-cell="${thNameEl.innerText}">
                          <input type="text" name="names[]" class="form-field"
                            placeholder="Name" required>
                      </td>
                      <td data-cell="${thKeyEl.innerText}">
                          <input type="text" name="keys[]" class="form-field"
                            placeholder="Key" required>
                      </td>
                      <td data-cell="${thValueEl.innerText}">
                          <input type="text" name="values[]" class="form-field"
...
function changeBackground(element, bgs) {
    logFunction(`changeBackground`, JSON.stringify({ element, bgs }));
    ...
    if (element) {
        ...
        const newBg = getRandomArrayElement(bgs);
        element.classList.remove(oldBg);
